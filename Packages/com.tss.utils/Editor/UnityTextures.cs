...
namespace TSS.Utils.Editor
{
    public class UnityTextures : EditorWindow
    {
        ...
        static void Init ()
        {
            UnityTextures window = EditorWindow.GetWindow<UnityTextures>("Unity Textures", typeof(SceneView));
            window.Show();
        }
        ...
        {
            GUILayout.Space(15f);
            EditorGUILayout.HelpBox("Klikom na dugme sa ikonicom mozete kopiratu njenu vrednost u clipboard", MessageType.Info);
            GUILayout.Space(15f);
            ...
            foreach (string path in pathList)
            {
                if (string.IsNullOrEmpty(path))
                    continue;
                if (path.ToUpper().Contains(filter.ToUpper()))
                {
                    EditorGUILayout.BeginHorizontal();
//                Debug.Log ("<color=aqua>UNITY: </color>" + path);
                    GUIItem(path);
                    EditorGUILayout.EndHorizontal();
                }
            }
        ...
        void GUIItem (string path)
        {
            ...
            if (GUILayout.Button(m_btnCntent, GUILayout.Width(50)))
            {
                ...
                Debug.Log("UNITY: " + GUIUtility.systemCopyBuffer);
            }
